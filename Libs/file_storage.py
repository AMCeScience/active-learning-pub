...
def load_results(review_name, run_similarity = False, similarity_size = None):
  dataset_name = ('review_%s' % review_name)
  ...
  filename = config.RESULTS_LOCATION + '/' + dataset_name + '.pickle'

  if file_exists(filename):
    return read_file(filename)['results']
...
def load_result_item(name):
  filename = config.RESULTS_LOCATION + '/%s.pickle' % name
...
def store_result_item(name, item):
  with open(config.RESULTS_LOCATION + '/%s.pickle' % name, 'wb') as handle:
